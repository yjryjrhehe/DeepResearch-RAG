...
class MarkdownSplitter(TextSplitter):
    ...
    def _split_sync(self, markdown_content: str, source: DocumentSource) -> List[DocumentChunk]:
        """同步实现：便于放入线程池执行，避免阻塞事件循环。"""
        ...
        for i, md_chunk in enumerate(initial_md_chunks):
            ...
            # 长度超标，递归分割
            log.warning(f"块 {i} (Token: {chunk_token_length}) 超出阈值，进行二次分割。")
            try:
                sub_chunks: List[Document] = self._recursive_splitter.split_documents(
                    [md_chunk]
                )
            except Exception as e:
    ...
    async def split(self, markdown_content: str, source: DocumentSource) -> List[DocumentChunk]:
        """
        将 Markdown 文本分割成 DocumentChunk 列表（异步包装）。
        """
        return await asyncio.to_thread(self._split_sync, markdown_content, source)
